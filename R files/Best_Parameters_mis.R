...
# for the model with attrition. 
# Author: Martin Wiegand
# Last changed: 23.09.2020

Best_Parameters_mis <- function(method, data, y, d, r, rt, rc, groupvar, xvars, folds, reps, seed = 1){
  ...
  for (r in 1:reps) {
    ...
    for (i in 1:folds) {
      ...
      indexlist.d1[[counter]] <- intersect(which(foldslist.d[[r]]!=i),d.is.1.r.is.1)
      indexlist.d0[[counter]] <- intersect(which(foldslist.d[[r]]!=i),d.is.0.r.is.1)
      ...
      indexoutlist.d1[[counter]] <- intersect((1:nrow(data))[-indexlist.d[[counter]]],d.is.1.r.is.1)
      indexoutlist.d0[[counter]] <- intersect((1:nrow(data))[-indexlist.d[[counter]]],d.is.0.r.is.1)
      indexoutlist.rt[[counter]] <- (1:nrow(data))[-indexlist.d[[counter]]]
      indexoutlist.rc[[counter]] <- (1:nrow(data))[-indexlist.d[[counter]]]
      indexoutlist.r_t[[counter]] <- intersect((1:nrow(data))[-indexlist.d[[counter]]],d.is.1)
      indexoutlist.r_c[[counter]] <- intersect((1:nrow(data))[-indexlist.d[[counter]]],d.is.0)
      counter <- counter + 1
    }
  }
  ...
  var.not.0 <- var.not.0.d0.r1 * var.not.0.d1.r1 # indices of variables with non-zero variance for both d=0 and d=1
  ...
  if(method == "Lasso" | method == "PostLasso" | method == "Ridge") {
    ...
    lambda.seq.r_c <- NULL
    for(r in 1:reps) {
      lasso.yd0 <- cv.glmnet(x=data.d0, y=y.d0, family="binomial", lambda=lambda.seq.yd0, alpha=alp, foldid = foldslist.d0[[r]], parallel = TRUE) 
      if (r==1) {
      ...
      lasso.yd1 <- cv.glmnet(x=data.d1, y=y.d1, family="binomial", lambda=lambda.seq.yd1, alpha=alp, foldid = foldslist.d1[[r]], parallel = TRUE)
      if (r==1) {
      ...
      lasso.rt <- cv.glmnet(x=data.d, y=y.rt, family="binomial", lambda=lambda.seq.rt, alpha=alp, foldid = foldslist.rt[[r]], parallel = TRUE)
      if (r==1) {
        while (lasso.rt$lambda.min >= lasso.rt$lambda[5] & lasso.rt$cvm[1] != lasso.rt$cvm[length(lasso.rt$cvm)] & lasso.rt$lambda[5]<100000) {
          lambda.seq.rt <- lasso.rt$lambda*10
          lasso.rt <- cv.glmnet(x=data.d, y=y.rt, family="binomial", lambda=lambda.seq.rt, alpha=alp, foldid = foldslist.rt[[r]], parallel = TRUE)
        }
        lambda.seq.rt <- lasso.rt$lambda
      }
      lasso.rc <- cv.glmnet(x=data.d, y=y.rc, family="binomial", lambda=lambda.seq.rc, alpha=alp, foldid = foldslist.rc[[r]], parallel = TRUE)
      if (r==1) {
        while (lasso.rc$lambda.min >= lasso.rc$lambda[5] & lasso.rc$cvm[1] != lasso.rc$cvm[length(lasso.rc$cvm)] & lasso.rc$lambda[5]<100000) {
          lambda.seq.rc <- lasso.rc$lambda*10
          lasso.rc <- cv.glmnet(x=data.d, y=y.rc, family="binomial", lambda=lambda.seq.rc, alpha=alp, foldid = foldslist.rc[[r]], parallel = TRUE)
        }
        lambda.seq.rc <- lasso.rc$lambda
      }
      lasso.r_t <- cv.glmnet(x=data.r_t, y=y.r_t, family="binomial", lambda=lambda.seq.r_t, alpha=alp, foldid = foldslist.r_t[[r]], parallel = TRUE)
      if (r==1) {
        while (lasso.r_t$lambda.min >= lasso.r_t$lambda[5] & lasso.r_t$cvm[1] != lasso.r_t$cvm[length(lasso.r_t$cvm)] & lasso.r_t$lambda[5]<100000) {
          lambda.seq.r_t <- lasso.r_t$lambda*10
          lasso.r_t <- cv.glmnet(x=data.r_t, y=y.r_t, family="binomial", lambda=lambda.seq.r_t, alpha=alp, foldid = foldslist.r_t[[r]], parallel = TRUE)
        }
        lambda.seq.r_t <- lasso.r_t$lambda
      }
      lasso.r_c <- cv.glmnet(x=data.r_c, y=y.r_c, family="binomial", lambda=lambda.seq.r_c, alpha=alp, foldid = foldslist.r_c[[r]], parallel = TRUE)
      if (r==1) {
        while (lasso.r_c$lambda.min >= lasso.r_c$lambda[5] & lasso.r_c$cvm[1] != lasso.r_c$cvm[length(lasso.r_c$cvm)] & lasso.r_c$lambda[5]<100000) {
    ...
    para <- list(lambda.yd0=lambda.yd0, lambda.yd1=lambda.yd1, lambda.d=lambda.d, lambda.rt=lambda.rt, lambda.rc=lambda.rc, lambda.r_t=lambda.r_t, lambda.r_c=lambda.r_c)
  ...
  if(method == "Elnet") {
    ...
    for(alp in alphas) {
      ...
      lambdamat[4,which(alphas == alp)] <- lasso.rt$lambda[which.min(cv.error.rt)]
      lambdamat[5,which(alphas == alp)] <- lasso.rc$lambda[which.min(cv.error.rc)]
      lambdamat[6,which(alphas == alp)] <- lasso.r_t$lambda[which.min(cv.error.r_t)]
      lambdamat[7,which(alphas == alp)] <- lasso.r_c$lambda[which.min(cv.error.r_c)]
    ...
    alpha.rt <- alphas[which(errormat == rowMins(errormat, rows = 4), arr.ind = TRUE)[2]]
    lambda.rt <- lambdamat[4, which(errormat == rowMins(errormat, rows = 4), arr.ind = TRUE)[2]]
    alpha.rc <- alphas[which(errormat == rowMins(errormat, rows = 5), arr.ind = TRUE)[2]]
    lambda.rc <- lambdamat[5, which(errormat == rowMins(errormat, rows = 5), arr.ind = TRUE)[2]]
    alpha.r_t <- alphas[which(errormat == rowMins(errormat, rows = 6), arr.ind = TRUE)[2]]
    lambda.r_t <- lambdamat[6, which(errormat == rowMins(errormat, rows = 6), arr.ind = TRUE)[2]]
    alpha.r_c <- alphas[which(errormat == rowMins(errormat, rows = 7), arr.ind = TRUE)[2]]
    lambda.r_c <- lambdamat[7, which(errormat == rowMins(errormat, rows = 7), arr.ind = TRUE)[2]]
    
    para <- list(lambda.yd0=lambda.yd0, lambda.yd1=lambda.yd1, lambda.d=lambda.d, lambda.rt=lambda.rt, lambda.rc=lambda.rc, lambda.r_t=lambda.r_t, lambda.r_c=lambda.r_c, 
                 alpha.yd0=alpha.yd0, alpha.yd1=alpha.yd1, alpha.d=alpha.d, alpha.rt=alpha.rt, alpha.rc=alpha.rc, alpha.r_t=alpha.r_t, alpha.r_c=alpha.r_c)
  ...
  if(method == "SVM") {
    ...
    paragrid <- expand.grid(C = costvector, sigma = gammavector) # the names vary across functions...
    ...
    svm_trcontrol.d0 <- trainControl(index = indexlist.d0, indexOut = indexoutlist.d0, indexFinal = d.is.0.r.is.1,
                                     verboseIter = TRUE, returnData = FALSE, summaryFunction=twoClassSummary,	
                                     classProbs=TRUE, returnResamp = "all", allowParallel = TRUE)
    
    svm.tune.d0 <- train(x = data.y, y = y.y, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
                         scale = FALSE, trControl=svm_trcontrol.d0, nthread=1)$bestTune
    
    svm_trcontrol.d1 <- trainControl(index = indexlist.d1, indexOut = indexoutlist.d1, indexFinal = d.is.1.r.is.1,
                                     verboseIter = TRUE, returnData = FALSE, summaryFunction=twoClassSummary,	
                                     classProbs=TRUE, returnResamp = "all", allowParallel = TRUE)
    
    svm.tune.d1 <- train(x = data.y, y = y.y, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
    ...
    svm_trcontrol.d <- trainControl(index = indexlist.d, indexOut = indexoutlist.d, 
                                    verboseIter = TRUE, returnData = FALSE, summaryFunction=twoClassSummary,	
                                    classProbs=TRUE, returnResamp = "all", allowParallel = TRUE)
    svm.tune.d <- train(x = data.y, y = y.d, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
                        scale = FALSE, trControl=svm_trcontrol.d, nthread=1)$bestTune
    
    svm_trcontrol.rt <- trainControl(index = indexlist.rt, indexOut = indexoutlist.rt, 
                                     verboseIter = TRUE, returnData = FALSE, summaryFunction=twoClassSummary,	
                                     classProbs=TRUE, returnResamp = "all", allowParallel = TRUE)
    svm.tune.rt <- train(x = data.y, y = y.rt, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
                         scale = FALSE, trControl=svm_trcontrol.rt, nthread=1)$bestTune
    
    svm_trcontrol.rc <- trainControl(index = indexlist.rc, indexOut = indexoutlist.rc, 
                                     verboseIter = TRUE, returnData = FALSE, summaryFunction=twoClassSummary,	
                                     classProbs=TRUE, returnResamp = "all", allowParallel = TRUE)
    svm.tune.rc <- train(x = data.y, y = y.rc, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
                         scale = FALSE, trControl=svm_trcontrol.rc, nthread=1)$bestTune
    
    svm_trcontrol.r_t <- trainControl(index = indexlist.r_t, indexOut = indexoutlist.r_t, indexFinal = d.is.1,
                                      verboseIter = TRUE, returnData = FALSE, summaryFunction=twoClassSummary,	
                                      classProbs=TRUE, returnResamp = "all", allowParallel = TRUE)
    svm.tune.r_t <- train(x = data.y, y = y.rt, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
                          scale = FALSE, trControl=svm_trcontrol.r_t, nthread=1)$bestTune
    
    svm_trcontrol.r_c <- trainControl(index = indexlist.r_c, indexOut = indexoutlist.r_c, indexFinal = d.is.0,
                                      verboseIter = TRUE, returnData = FALSE, summaryFunction=twoClassSummary,	
                                      classProbs=TRUE, returnResamp = "all", allowParallel = TRUE)
    svm.tune.r_c <- train(x = data.y, y = y.rc, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
                          scale = FALSE, trControl=svm_trcontrol.r_c, nthread=1)$bestTune
  ...
  if(method == "Boosting") {
    ...
    xgb_trcontrol.d0 <- trainControl(index = indexlist.d0, indexOut = indexoutlist.d0, indexFinal = d.is.0.r.is.1, 
                                     verboseIter = TRUE, returnData = FALSE, returnResamp = "all", 
                                     classProbs = TRUE, summaryFunction = twoClassSummary, allowParallel = TRUE)
    
    xgb_trcontrol.d1 <- trainControl(index = indexlist.d1, indexOut = indexoutlist.d1, indexFinal = d.is.1.r.is.1, 
                                     verboseIter = TRUE, returnData = FALSE, returnResamp = "all", 
                                     classProbs = TRUE, summaryFunction = twoClassSummary, allowParallel = TRUE)
    
    xgb_trcontrol.rt <- trainControl(index = indexlist.rt, indexOut = indexoutlist.rt, 
                                     verboseIter = TRUE, returnData = FALSE, returnResamp = "all", 
                                     classProbs = TRUE, summaryFunction = twoClassSummary, allowParallel = TRUE)
    
    xgb_trcontrol.rc <- trainControl(index = indexlist.rc, indexOut = indexoutlist.rc, 
                                     verboseIter = TRUE, returnData = FALSE, returnResamp = "all", 
                                     classProbs = TRUE, summaryFunction = twoClassSummary, allowParallel = TRUE)
    
    xgb_trcontrol.r_t <- trainControl(index = indexlist.r_t, indexOut = indexoutlist.r_t, indexFinal = d.is.1, 
                                      verboseIter = TRUE, returnData = FALSE, returnResamp = "all", 
                                      classProbs = TRUE, summaryFunction = twoClassSummary, allowParallel = TRUE)
    
    xgb_trcontrol.r_c <- trainControl(index = indexlist.r_c, indexOut = indexoutlist.r_c, indexFinal = d.is.0, 
                                      verboseIter = TRUE, returnData = FALSE, returnResamp = "all", 
                                      classProbs = TRUE, summaryFunction = twoClassSummary, allowParallel = TRUE)
    
    xgb_train_d0.1 <- train(x = data.y, y = y.y, trControl = xgb_trcontrol.d0, 
    ...
    xgb_grid.2.rt <- expand.grid(nrounds = nrounds.rt, eta = eta.rt, 
                                 max_depth = max_depth.rt, gamma = 0, subsample = c(0.4,0.7,1),
                                 colsample_bytree = c(0.5,0.7,1), min_child_weight = c(0.5,1,2,4))
    
    xgb_grid.2.rc <- expand.grid(nrounds = nrounds.rc, eta = eta.rc, 
                                 max_depth = max_depth.rc, gamma = 0, subsample = c(0.4,0.7,1),
                                 colsample_bytree = c(0.5,0.7,1), min_child_weight = c(0.5,1,2,4))
    
    xgb_grid.2.r_t <- expand.grid(nrounds = nrounds.r_t, eta = eta.r_t, 
                                  max_depth = max_depth.r_t, gamma = 0, subsample = c(0.4,0.7,1),
                                  colsample_bytree = c(0.5,0.7,1), min_child_weight = c(0.5,1,2,4))
    
    xgb_grid.2.r_c <- expand.grid(nrounds = nrounds.r_c, eta = eta.r_c, 
                                  max_depth = max_depth.r_c, gamma = 0, subsample = c(0.4,0.7,1),
                                  colsample_bytree = c(0.5,0.7,1), min_child_weight = c(0.5,1,2,4))
    
    
    # train the model for each parameter combination in the grid, using CV to evaluate
    
    xgb_train_d0.2 <- train(x = data.y, y = y.y, trControl = xgb_trcontrol.d0, 
                            tuneGrid = xgb_grid.2.d0, method = "xgbTree", nthread=1)$bestTune
    
    xgb_train_d1.2 <- train(x = data.y, y = y.y, trControl = xgb_trcontrol.d1, 
                            tuneGrid = xgb_grid.2.d1, method = "xgbTree", nthread=1)$bestTune
    
    xgb_train_d.2 <- train(x = data.y, y = y.d, trControl = xgb_trcontrol.d, 
                           tuneGrid = xgb_grid.2.d, method = "xgbTree", nthread=1)$bestTune
    
    xgb_train_rt.2 <- train(x = data.y, y = y.rt, trControl = xgb_trcontrol.rt, 
                            tuneGrid = xgb_grid.2.rt, method = "xgbTree", nthread=1)$bestTune
    
    xgb_train_rc.2 <- train(x = data.y, y = y.rc, trControl = xgb_trcontrol.rc, 
                            tuneGrid = xgb_grid.2.rc, method = "xgbTree", nthread=1)$bestTune
    
    xgb_train_r_t.2 <- train(x = data.y, y = y.rt, trControl = xgb_trcontrol.r_t, 
                             tuneGrid = xgb_grid.2.r_t, method = "xgbTree", nthread=1)$bestTune
    
    xgb_train_r_c.2 <- train(x = data.y, y = y.rc, trControl = xgb_trcontrol.r_c, 
