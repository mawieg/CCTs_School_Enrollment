...
# for the model without attrition. 
# Author: Martin Wiegand
# Last changed: 23.09.2020

Best_Parameters <- function(method, data, y, d, groupvar, xvars, folds, reps, seed = 1){
  ...
  if(method == "SVM") {
    ...
    data.y <- dplyr::select(data, one_of(x)) %>%
      mutate(ynew = factor(data[,y]*2 - 1, labels=c("X0", "X1")))
    ...
    svm.tune.d0 <- train(form = form, data = data.y, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
    ...
    svm.tune.d1 <- train(form = form, data = data.y, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
    ...
    svm.tune.d <- train(form = form, data = data.d, method = "svmRadial", metric = "ROC", tuneGrid=paragrid, 
