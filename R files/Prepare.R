# This program prepares the conditional sample datasets for the DML analysis. In particular,
...
if (set=="big"){
if (status == "poor") {
  data <- read_csv("Highschool_big.csv", na = c("", "NA", "99", "999", "9999")) %>%
    filter(pobre == 'Pobre') %>%
    rename(statusqualifier = verypoor) %>%
    mutate(outcome = get(outcomename), R03 = (1-missing03), R03.t = (1-missing03)*treatment, R03.c = (1-missing03)*(1-treatment)) %>%
    dplyr::select(-pobre, -poor, -highschool, -highschool03, -highschool03fin, -moneyreason, -otherreason, -missing03, -pcexp, -lnpcexp)
}
...
if (set=="small"){
  if (status == "poor") {
    data <- read_csv("Highschool_small.csv", na = c("", "NA", "99", "999", "9999")) %>%
      filter(pobre == 'Pobre') %>%
      rename(statusqualifier = verypoor) %>%
      mutate(outcome = get(outcomename), R03 = (1-missing03), R03.t = (1-missing03)*treatment, R03.c = (1-missing03)*(1-treatment)) %>%
      dplyr::select(-pobre, -poor, -highschool, -highschool03, -highschool03fin, -moneyreason, -otherreason, -missing03, -pcexp, -lnpcexp)
...
interactionsvars <- c("yycali", "mpcalif", "numunder15", "fatherathome", "motherathome", "fatherlit", "motherlit", "fatherschool", "motherschool", "fatherminprim", "motherminprim", "fatherminsec", "motherminsec", "statusqualifier", "goodatschool", "cansec", "canfurther", "canhigh", "canuni", "childlabor", "avchildwage", "indice", "grado", "female", "age97", "Miss.avchildwage", "indloc", "attend97", "attend98", "vilcount", "rightagetotal", "rightagepoortotal", "rightageattend97total", "rightageattend97poortotal")
...
for (name in names(xo)){
  if (anova(lm(databig[,name] ~ factor(databig$locid), na.action=na.omit))$`F value`[1] > 20) {
    nogroupvar <- c(nogroupvar, 0)
  } else {
    nogroupvar <- c(nogroupvar, 1)
  }
...
# Assign variable firstobs, assigning 1 to the forst observation in each cluster and 0 otherwise, 
# abd variable weight, indicating the number of observations per cluster (only accurate for firstobs==1).
# These are being used to create a small version of the dataset, with only one observation per cluster.  
databig$firstobs <- 1
databig$weight <- 1
counter <- 0
