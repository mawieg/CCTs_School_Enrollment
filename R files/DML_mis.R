...
# learning, using outcome, treatment status, indicators for missing outcomes, 
# covariates, and ML methods as inputs.
...
# - R.squared.y, R.squared.d, R.squared.r: lists (of R2 for each ML method)
...
DML_mis <- function(data, y, d, r, rt, rc, groupvar, xx, xo, xL, methods, nfold, 
                    arguments, outcomename, silent=FALSE){
  ...
  for(k in 1:M){   
    ...
    for(j in 1:K){ 
      ...
      var.not.0.d0 <- 
        !unlist(lapply(Xuse.d0.r1, function(x) length(unique(x)) <2)) 
      var.not.0.d1 <- 
        !unlist(lapply(Xuse.d1.r1, function(x) length(unique(x)) <2))
      ...
      remove(d.is.1, Xuse.d0, Xuse.d1, d.is.1.r.is.1, d.is.0.r.is.1, Xuse.d0.r1, 
             Xuse.d1.r1, var.not.0.d0, var.not.0.d1, var.not.0)
      
        
      cond.comp[[k,j]] <- cond_comp_mis(datause=datause, dataout=dataout, y, d, 
                                        r, rt, rc, form_x, methods[k], 
                                        plinear=0, binary.d, binary.y, 
                                        arguments=arguments)
      # MSE of g(x,0)
      MSE1[k] <- MSE1[k] + 
        cond.comp[[k,j]]$err.yz0^2 * 
        length(which(dataout[,d]==0 & dataout[,r]==1)) / 
        length(which(data[,d]==0 & data[,r]==1))
      # MSE of g(x,1)
      MSE2[k] <- MSE2[k] + 
        cond.comp[[k,j]]$err.yz1^2 * 
        length(which(dataout[,d]==1 & dataout[,r]==1)) / 
        length(which(data[,d]==1 & data[,r]==1))  
      # MSE of m(x) (prop. score)
      MSE3[k] <- MSE3[k] + 
        cond.comp[[k,j]]$err.z^2 * dim(dataout)[1] / dim(data)[1]
      # MSE of m(x)*r(1,x)
      MSE4[k] <- MSE4[k] + 
        cond.comp[[k,j]]$err.rt^2 * dim(dataout)[1] / dim(data)[1]
      # MSE of m(x)*r(0,x)
      MSE5[k] <- MSE5[k] + 
        cond.comp[[k,j]]$err.rc^2 * dim(dataout)[1] / dim(data)[1] 
      # MSE of r(1,x)  
      MSE6[k] <- MSE6[k] + 
        cond.comp[[k,j]]$err.r_t^2 * length(which(dataout[,d]==1)) / 
        length(which(data[,d]==1))
      # MSE of r(0,x)     
      MSE7[k] <- MSE7[k] + 
        cond.comp[[k,j]]$err.r_c^2 * length(which(dataout[,d]==0)) / 
  ...
  for (j in 1:K) {
    ...
    # MSE of g(x,0)
    MSE1[M+1] <- MSE1[M+1] + 
      cond.comp[[M+1,j]]$err.yz0^2 * 
      length(which(dataout[,d]==0 & dataout[,r]==1)) / 
      length(which(data[,d]==0 & data[,r]==1))  
    # MSE of g(x,1)
    MSE2[M+1] <- MSE2[M+1] + 
      cond.comp[[M+1,j]]$err.yz1^2 * 
      length(which(dataout[,d]==1 & dataout[,r]==1)) / 
      length(which(data[,d]==1 & data[,r]==1))  
    # MSE of m(x) (prop. score)
    MSE3[M+1] <- MSE3[M+1] + 
    ...
    # MSE of m(x)*r(1,x)
    MSE4[M+1] <- MSE4[M+1] + 
      cond.comp[[M+1,j]]$err.rt^2 * dim(dataout)[1] / dim(data)[1] 
    # MSE of m(x)*r(0,x)
    MSE5[M+1] <- MSE5[M+1] + 
      cond.comp[[M+1,j]]$err.rc^2 * dim(dataout)[1] / dim(data)[1] 
    # MSE of r(1,x)
    MSE6[M+1] <- MSE6[M+1] + 
      cond.comp[[M+1,j]]$err.r_t^2 * length(which(dataout[,d]==1)) / 
      length(which(data[,d]==1))    
    # MSE of r(0,x)
    MSE7[M+1] <- MSE7[M+1] + 
      cond.comp[[M+1,j]]$err.r_c^2 * length(which(dataout[,d]==0)) / 
      length(which(data[,d]==0))       
  }
  
  R.squared.y <- R.squared.d <- R.squared.r <- keepvec <- trimvec <- 
    rep(0,(M+1))
  ...
  for (k in (1:(M+1))) {
    ...
    for(t in 1:3){ # t = ATE, ATET, ATEN
      ...
      for(j in 1:K) {
        ...
        if (t==1) {keep <- c(NA,1)[(cond.comp[[k,j]]$m_rt>trim & 
                                      cond.comp[[k,j]]$m_rc>trim) + 1]}
        if (t==2) {keep <- c(NA,1)[(cond.comp[[k,j]]$m_r_t>trim & 
                                      cond.comp[[k,j]]$m_rc>trim) + 1]}
        if (t==3) {keep <- c(NA,1)[(cond.comp[[k,j]]$m_rt>trim & 
                                      cond.comp[[k,j]]$m_r_c>trim) + 1]}
        ...
        # some of these are NA already, meaning these outcomes are unobserved
        yout <- c(yout, dataout[,y]) 
      ...
      if (t == 1) {
        TE[,k] <- (rout * dout * (yout - my_z1x) / m_rt) - 
          (rout * (1 - dout) * (yout - my_z0x) / m_rc) + my_z1x - my_z0x;
      }
      if (t == 2) {
        TET[,k] <- (rout * dout * (yout - my_z1x) / (pout * m_r_t) ) - 
          (rout * (1 - dout) * (yout - my_z0x) * mz_x) / (pout * m_rc) + 
          (dout/pout) * (my_z1x - my_z0x);
      }
      if (t == 3) {
        TEN[,k] <- (rout * dout * (yout - my_z1x) * (1 - mz_x) / 
                      ((1 - pout) * m_rt) ) - 
          (rout * (1 - dout) * (yout - my_z0x)) / ((1 - pout) * m_r_c) + 
          ((1 - dout) / (1 - pout)) * (my_z1x - my_z0x);
      }
      ...
      res.sum.of.squares.y <- 
        sum((yout[rout==1] - yhat[rout==1])^2, na.rm = TRUE)
  ...
  RMSE <- rbind(sqrt(MSE1), sqrt(MSE2), sqrt(MSE3), sqrt(MSE4), sqrt(MSE5), 
                sqrt(MSE6), sqrt(MSE7))
  rownames(RMSE) <- 
    c("RMSE[Y|X, D=0]", "RMSE[Y|X, D=1]", "RMSE[D|X]", "RMSE[RD|X]", 
      "RMSE[R(1-D)|X]", "RMSE[R|X, D=1]", "RMSE[R|X, D=0]")
  
  result.list <- list(TE, TET, TEN, groupout, RMSE, R.squared.y, R.squared.d, 
                      R.squared.r, keepvec, trimvec)
  names(result.list) <- c("TE", "TET", "TEN", "groupID", "RMSE", "R.squared.y", 
                          "R.squared.d", "R.squared.r", "keepvec", "trimvec")
  return(result.list)
}  
