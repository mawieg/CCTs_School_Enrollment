# This program prepares the unconditional sample datasets for the DML analysis. 
# In particular, it creates the predictors to be used by the various machine 
# learning methods. 
...
rc       <- "R03.c";
groupvar <- "locid";
# declare missing value indicator variable
...
if (status == "poor") {
  data <- read_csv("Intermediate data/Unconditional.csv", 
                   na = c("", "NA", "99", "999", "9999")) %>%
    filter(pobre == 'Pobre') %>%
    rename(statusqualifier = verypoor) %>%
    mutate(outcome = get(outcomename), R03 = (1-get(whichmis)), 
           R03.t = (1-get(whichmis))*treatment, 
           R03.c = (1-get(whichmis))*(1-treatment)) %>%
    dplyr::select(-pobre, -poor, -highschool, -highschool03, -highschool03fin, 
                  -missing00, -missing03, -middleschool00, -somemiddleschool00, 
                  -missingmiddle00)
}
...
for (name in names(xx)){
  if (anova(lm(databig[,name] ~ factor(databig$locid)))$`Pr(>F)`[1] < 0.00001 & 
      length(unique(databig[,name])) > 3) {
...
for (name in names(xo)){
  if (anova(lm(databig[,name] ~ factor(databig$locid), 
               na.action=na.omit))$`Pr(>F)`[1] < 0.00001 & 
      length(unique(databig[,name])) > 2) {
    nogroupvar <- c(nogroupvar, 0)
  } else {
    nogroupvar <- c(nogroupvar, 1)
  }
