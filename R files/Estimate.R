# This function runs the DML precedure with non-missing outcomes for a number of prespecified 
...
large.list <- foreach(ite = 1:iterations, .errorhandling='remove', .packages = c('tidyverse', 'randomForest', 'partykit', 'Matching', 'CausalGAM', 'survey', 'glmnet', 'e1071', 'caret', 'splines2', 'foreign', 'quantreg', 'mnormt', 'gbm', 'glmnet', 'MASS', 'rpart', 'doParallel', 'sandwich', 'hdm', 'matrixStats', 'quadprog', 'lsei', 'xgboost', 'groupdata2', 'multiwayvcov')) %dopar% { 
  set.seed(12345+ite)
  res <- DML(databig, y, d, groupvar, xx, xo, xL, methods=methods, nfold=nfolds, arguments=arguments, arguments.ens=arguments, outcomename=outcomename, silent=FALSE)
  res <- res
}

result      <- matrix(0,9, length(methods)+1)
colnames(result) <- cbind(t(methods), "Best")
rownames(result) <- cbind("Median ATE", "bse", "pval", "Median ATEN", "bse", "pval", "RMSE[Y|X, D=0]", "RMSE[Y|X, D=1]", "RMSE[D|X]")
