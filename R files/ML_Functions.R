# This function contains various programs used for the calculation of DML 
# treatment effects, including the actual machine learning algorithms to predict 
# nuisance parameters, score functions, and further auxiliary functions. 
...
RegRegF <- function(datause, dataout, form_x, form_y, logit=FALSE, 
                    lambda=lambda, alp=alp, post=FALSE){
  form            <- as.formula(paste(form_y, "~", form_x));
  Xuse             <- as.matrix(dplyr::select(datause, 
                                              one_of(all.vars(form)[-1])))
  yuse             <- dplyr::select(datause, one_of(all.vars(form)[1]))[[1]]
  Xout            <- as.matrix(dplyr::select(dataout, 
                                             one_of(all.vars(form)[-1])))
  ...
  if(post==FALSE){
    ...
    for (l in 1:length(lambda)) {
      ...
      yhatuse         <- cbind(yhatuse, predict(postlasso, newdata=datause, 
                                                type="response"))
    }
  }
  resuse          <- 1 
  ...
  return(list(yhatuse = yhatuse, resuse=resuse, yhatout = yhatout, 
              resout=resout, xuse=Xuse, xout=Xout, model=model, yout=yout, 
              form=form));
}

# Neural Networks (ended up not using)
nnetF <- function(datause, dataout, form_x, form_y, clas=FALSE, size, decay){
  linout=FALSE
  if(clas==TRUE){ linout=FALSE}
  form           <- as.formula(paste(form_y, "~", form_x))
  nn             <- nnet(formula=form, data=datause, linout=linout, size=size, 
                         decay=decay, maxit=1000, MaxNWts=50000, trace=FALSE)
  ...
  return(list(yhatuse = yhatuse, resuse=resuse, yhatout = yhatout, 
              resout=resout, xuse=Xuse, xout=Xout, model=nn));
}

# Extreme gradient boosting
xg.boost <- function(datause, dataout, form_x, form_y, nround, eta, max.depth, 
                     subsample, colsample_bytree, min_child_weight, binary=0, 
                     double=FALSE, startmodel=0){
  form      <- as.formula(paste(form_y, "~", form_x));
  Xuse      <- as.matrix(dplyr::select(datause, one_of(all.vars(form)[-1])))
  yuse      <- as.matrix(dplyr::select(datause, one_of(all.vars(form)[1]))[[1]])
  if (binary==1) {
    boostfit <- xgboost::xgboost(data = Xuse, label = yuse, 
                                 max.depth = max.depth, eta = eta, 
                                 nround = nround, subsample = subsample, 
                                 colsample_bytree = colsample_bytree, 
                                 min_child_weight = min_child_weight, 
                                 objective = "binary:logistic", verbose = FALSE, 
                                 save_period = NULL, nthread = 1)
  } else {
    boostfit <- xgboost::xgboost(data = Xuse, label = yuse, 
                                 max.depth = max.depth, eta = eta, 
                                 nround = nround, subsample = subsample, 
  ...
  resout         <- yout - yhatout
  return(list(yhatuse = yhatuse, resuse=resuse, yhatout = yhatout, 
              resout=resout, xuse=Xuse, xout=Xout, model=boostfit));
}

# Random forest
RF <- function(datause, dataout,  form_x, form_y, x=NA, y=NA, xout=NA, yout=NA, 
               nodesize, arg, reg=FALSE, tune=FALSE){
  yhatout <- NA
  reuse   <- NA
  yhatuse <- NA
  resout  <- NA
  ...
  if(tune==FALSE){
      forest      <- do.call(randomForest, append(list(formula=form, 
                                                       nodesize=nodesize, 
                                                       data=datause), arg))
  }
...
svmF <- function(datause, dataout, form_x, form_y, cost, gamma, binary){
  ...
  if(binary==1){
    svmfit     <- svm(x=Xuse, y=as.factor(yuse), kernel = "radial", cost = cost, 
                      gamma = gamma, probability = TRUE, fitted = TRUE)
  ...
  if(binary==0){
    svmfit     <- svm(x=Xuse, y=yuse, kernel = "radial", cost = cost, 
                      fitted = TRUE)
  ...
  return(list(yhatuse = yhatuse, resuse=resuse, yhatout = yhatout, 
              resout=resout, xuse=Xuse, xout=Xout, model=svmfit));
}


# Score functions for treatment effects and their standard errors (analytical 
# and bootstrap)
...
BSSE.ATE <- function(y, d, my_d1x, my_d0x, md_x, groupid, B){
  ...
  SE <- sqrt(cluster.boot(helpmodel, groupid, R = B, boot_type = "wild", 
                          wild_type = "norm"))[1,1]
  return(SE);
}
...
BSSE.ATET <- function(y, d, my_d0x, md_x, p, groupid, B){
  depvar <- 
    (d * (y - my_d0x) / p) - ((md_x / (1 - md_x)) * (1 - d) * (y - my_d0x) / p)
  ones <- rep(1,length(depvar))
  helpmodel <- lm(depvar ~ ones)
  SE <- sqrt(cluster.boot(helpmodel, groupid, R = B, boot_type = "wild", 
                          wild_type = "norm"))[1,1]
...
BSSE.ATET_mis <- function(y, d, r, my_d1x, my_d0x, md_x, prc, pr_t, p, 
                          groupid, B){
  depvar <- 
    (r * d * (y - my_d1x) / (p * pr_t) ) - 
    (r * (1 - d) * (y - my_d0x) * md_x) / (p * prc) + 
...
ATEN_mis <- function(y, d, r, my_d1x, my_d0x, md_x, prt, pr_c, p){
  return( 
    mean( 
      (r * d * (y - my_d1x) * (1 - md_x) / ((1 - p) * prt) ) - 
        (r * (1 - d) * (y - my_d0x)) / ((1 - p) * pr_c) + 
        ((1 - d) / (1 - p)) * (my_d1x - my_d0x) 
    )  
  );
}
...
BSSE.ATEN_mis <- function(y, d, r, my_d1x, my_d0x, md_x, prt, pr_c, p, 
                          groupid, B){
  depvar <- 
    (r * d * (y - my_d1x) * (1 - md_x) / ((1 - p) * prt) ) - 
    (r * (1 - d) * (y - my_d0x)) / ((1 - p) * pr_c) + 
...
optimal.trim_mis <- function(prop.score.1, prop.score.2)
{
  ...
  if(max(score) <= 2*mean(score) & max(score) != Inf) {
    ...
    stop <- 0
    while(stop < 2 & i <= N) {
      if(((gamma / N) * sum(score <= gamma)) < 
         ((2 / N) * sum(score * (score <= gamma), na.rm = TRUE))) {
        stop <- 0
      } else {
        stop <- stop + 1
      }
...
# Fill missing values: for categorical variables, replace NAs by "Missing" 
# category; for numerical variables, create new indicator variables for missing 
# and replace NAs with average.
...
# This function fills NA in numerical cols by avg of those present and tacks on 
# extra columns for indicators of missing data. Returns data frame with results.
...
# Construct dataset that includes interactions and cubic terms of all numerical 
# variables.
# Based on code by Bob Stine.
...
build.predictors <- function(data, interactions) {
  ...
  int <- as.data.frame(
    model.matrix(~., data=data[intersect(interactions,colnames(data))])[,-1])
  inter <- names(int)
  for (var in inter) {
...
build.predictors2 <- function(data, interactions) {
  ...
  for (var in 1:contvars) {
    intermat <- bSpline(X[,var] + rnorm(contvars)*0.00000001, 
                        df = knots + degree + 1, degree = degree, 
                        intercept = TRUE)
    colnames(intermat) <- 
      paste(colnames(X)[var],".BS.",colnames(intermat),sep='')
    splines <- cbind(splines, intermat)
  }
  intersplinenames <- 
    paste(c('yycali', 'yycali', 'yycali', 'yycali', 'yycali', 'yycali', 
            'yycali', 'yycali', 'yycali', 
            'mpcalif', 'mpcalif', 'mpcalif', 'mpcalif', 'mpcalif', 'mpcalif', 
            'mpcalif', 'mpcalif', 'mpcalif'),".BS.",1:(knots+degree+1),sep='')
