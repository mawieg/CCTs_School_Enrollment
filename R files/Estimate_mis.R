...
for (k in 1:(M+1)) {
  ...
  RMSE.vector.rt <- rep(0, iterations)
  RMSE.vector.rc <- rep(0, iterations)
  RMSE.vector.r_t <- rep(0, iterations)
  RMSE.vector.r_c <- rep(0, iterations)
  for (ite in 1:iterations) {
    ...
    RMSE.vector.rt[ite] <- large.list[[ite]]$RMSE[4,k]
    RMSE.vector.rc[ite] <- large.list[[ite]]$RMSE[5,k]
    RMSE.vector.r_t[ite] <- large.list[[ite]]$RMSE[6,k]
    RMSE.vector.r_c[ite] <- large.list[[ite]]$RMSE[7,k]
  }
  result[7,k] <- median(RMSE.vector.y0)
  result[8,k] <- median(RMSE.vector.y1)
  result[9,k] <- median(RMSE.vector.d)
  result[10,k] <- median(RMSE.vector.rt)
  result[11,k] <- median(RMSE.vector.rc)
  result[12,k] <- median(RMSE.vector.r_t)
  result[13,k] <- median(RMSE.vector.r_c)
}
...
# Also compute R2 for non-attrition dummy
r2.r <- rep(0,(M+1))
for (ite in 1:iterations) {
  for (k in 1:(M+1)) {
    r2.r[k] <- r2.r[k] + large.list[[ite]]$R.squared.r[k]/iterations
  }
}
