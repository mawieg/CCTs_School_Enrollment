...
# the respective method for the model without attrition and continuous outcomes. 
...
Best_Parameters_Reg <- function(method, data, y, d, groupvar, xvars, folds, 
                                reps, seed = 1){
  ...
  if(method == "Lasso" | method == "PostLasso" | method == "Ridge") {
    ...
    lambda.seq.d <- NULL
    for(r in 1:reps) {
      lasso.yd0 <- cv.glmnet(x=data.d0, y=y.d0, family="gaussian", 
      ...
      lasso.yd1 <- cv.glmnet(x=data.d1, y=y.d1, family="gaussian", 
  ...
  if(method == "Elnet") {
    ...
    para <- list(lambda.yd0=lambda.yd0, lambda.yd1=lambda.yd1, 
                 lambda.d=lambda.d, alpha.yd0=alpha.yd0, alpha.yd1=alpha.yd1, 
                 alpha.d=alpha.d)
  ...
  if(method == "SVM") {
    ...
    paragrid.class <- expand.grid(C = costvector, sigma = gammavector) 
    ...
    svm.tune.d0 <- 
      train(form = form, data = data.y, method = "svmLinear", metric = "RMSE", 
            tuneGrid=paragrid.reg, scale = FALSE, trControl=svm_trcontrol.d0, 
            nthread=1)$bestTune
    ...
    svm.tune.d1 <- 
      train(form = form, data = data.y, method = "svmLinear", metric = "RMSE", 
            tuneGrid=paragrid.reg, scale = FALSE, trControl=svm_trcontrol.d1, 
            nthread=1)$bestTune
    ...
    svm.tune.d <- 
      train(form = form, data = data.d, method = "svmRadial", metric = "ROC", 
            tuneGrid=paragrid.class, scale = FALSE, trControl=svm_trcontrol.d, 
            nthread=1)$bestTune
  ...
  if(method == "Boosting") {
    ...
    xgb_train_d1.1 <- 
      train(x = data.x, y = y.y, trControl = xgb_trcontrol.d1, 
            tuneGrid = xgb_grid.1, method = "xgbTree", nthread=1)$bestTune
    ...
    xgb_train_d.1 <- 
      train(x = data.x, y = y.d, trControl = xgb_trcontrol.d, 
            tuneGrid = xgb_grid.1, method = "xgbTree", nthread=1)$bestTune
    ...
    # train the model for each parameter combination in the grid, using CV to 
    # evaluate
    
    xgb_train_d0.2 <- 
      train(x = data.x, y = y.y, trControl = xgb_trcontrol.d0, 
