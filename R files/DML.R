...
# using outcome, treatment status, covariates, and ML methods as inputs.
...
DML <- function(data, y, d, groupvar, xx, xo, xL, methods, nfold, arguments, arguments.ens, outcomename, silent=FALSE){
  ...
  # cvgroup is a random assignment of the observations into K groups (values are indices 1,...,K)
  ...
  for(k in 1:M){   # loop over methods
    ...
    # ignore variables with variance 0 in that fold
      ...
      Xuse.d0 <- dplyr::select(datause[-d.is.1,], one_of(xvars)) # predictors from training set
      Xuse.d1 <- dplyr::select(datause[d.is.1,], one_of(xvars))  # predictors from the left out fold
      ...
      cond.comp[[k,j]] <- cond_comp(datause=datause, dataout=dataout, y, d, form_x, methods[k], plinear=0, binary.d, binary.y, arguments=arguments)
      MSE1[k] <- MSE1[k] + cond.comp[[k,j]]$err.yz0^2 * length(which(dataout[,d]==0)) / length(which(data[,d]==0))  # MSE of g(x,0)
      MSE2[k] <- MSE2[k] + cond.comp[[k,j]]$err.yz1^2 * length(which(dataout[,d]==1)) / length(which(data[,d]==1))  # MSE of g(x,1)
  ...
  for (j in 1:K) {
    ...
    MSE1[M+1] <- MSE1[M+1] + cond.comp[[M+1,j]]$err.yz0^2 * length(which(dataout[,d]==0)) / length(which(data[,d]==0))  # MSE of g(x,0)
    MSE2[M+1] <- MSE2[M+1] + cond.comp[[M+1,j]]$err.yz1^2 * length(which(dataout[,d]==1)) / length(which(data[,d]==1))  # MSE of g(x,1)
  ...
  for (k in (1:(M+1))) {
    ...
    for(j in 1:K) {
      ii  <- cvgroup == j                  # ii index 1 for leftout group
      dataout <- as.data.frame(data[ii,])  # data split used to estimate treatment parameter (theta)
      keep <- c(NA,1)[(cond.comp[[k,j]]$mz_x>trim[1] & cond.comp[[k,j]]$mz_x<trim[2]) + 1]
    ...
    TE[,k] <- ((dout * (yout - my_z1x) / mz_x) -  ((1 - dout) * (yout - my_z0x) / (1 - mz_x)) + my_z1x - my_z0x)
    TET[,k] <- ((dout * (yout - my_z0x) / pout) - ((mz_x / (1 - mz_x)) * (1 - dout) * (yout - my_z0x) / pout))
    TEN[,k] <- (-(((1 - dout) * (yout - my_z1x) / (1 - pout)) - (((1 - mz_x) / mz_x) * dout * (yout - my_z1x) / (1 - pout))))
  ...
  result.list <- list(TE, TET, TEN, groupout, RMSE, R.squared.y, R.squared.d, keepvec, trimvec)
  names(result.list) <- c("TE", "TET", "TEN", "groupID", "RMSE", "R.squared.y", "R.squared.d", "keepvec", "trimvec")
  return(result.list)
}  
